  ...
  return (
    ...
    >
      {props.active ? (
        <span
          aria-hidden="true"
          className={cn("absolute inset-x-3 top-0 h-px bg-gradient-to-r opacity-80", activeAccent())}
        />
      ) : null}
      ...
      <span
        aria-hidden="true"
        className={cn(
          "relative flex size-11 shrink-0 items-center justify-center rounded-[1rem] border ring-1 transition-transform duration-200 group-hover:scale-[1.02]",
          toneStyles(props.active)
        )}
      >
        <Icon className="size-4" />
        {props.active ? <span className="absolute inset-0 rounded-[1rem] border border-slate-100 opacity-70" /> : null}
      </span>
...
export function SideNav(props: {
  ...
  return (
    <nav
      className={cn(
        "relative overflow-hidden rounded-[2.35rem] border border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] p-4 text-slate-950 shadow-[0_24px_60px_-44px_rgba(15,23,42,0.16)]",
        props.variant === "mobile" && "pb-6"
      )}
    >
      <div
        aria-hidden="true"
        ...
        {visibleGroups.map((group) => (
          <section key={group.title} className="space-y-3">
            <div className="flex items-end justify-between gap-3 px-1">
              <div>
                <p className="text-[0.68rem] font-semibold uppercase tracking-[0.42em] text-slate-500">
                  {group.title}
                </p>
